...
export async function createStory(story) {
  try {
    ...
    if(newStory) {
      await deleteStory(story.$id, uploadedFile.$id)
    }
...
export async function deleteStory(storyId, imageId) {
  ...
  try {
    ...
    const storyCreationTime = story.$createdAt

    const currentTime = Math.floor(Date.now()/1000)

    const timeDifference = currentTime - storyCreationTime;

    const TimeInSeconds = 5 * 60 * 60
    ...
    return {status: 'ok', message: 'story deleted sucessfully.'}
...
export async function deletePost(postId, imageId, videoId) {
  ...
  try {
    ...
    await deleteFile(imageId && videoId)
