...
export async function deleteStory(storyId, imageId) {
  try {
    if (!storyId) throw Error("Invalid story ID");

    // Fetch the Story creation time from your database
    ...
    const storyCreationTime = new Date(story.$createdAt).getTime() / 1000;
    const currentTime = Math.floor(Date.now() / 1000);

    const timeInSecondsThreshold = 2 * 60; // 5 hours threshold

    const timeDifference = currentTime - storyCreationTime;

    if (timeDifference >= timeInSecondsThreshold) {
      ...
      return { status: 'skipped', message: 'Not yet time to delete.' };
    }
  } catch (error) {
    console.error('Error deleting story:', error);
    return { status: 'error', message: 'Error deleting story.' };
  }
}
